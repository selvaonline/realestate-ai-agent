...
type Card =
  | { kind: 'status'|'wait'|'nav'|'action'|'fallback'; label: string; note?: string; url?: string; t: number }
  | { kind: 'shot'; label: string; b64: string; t: number }
  | { kind: 'extracted'; summary: any; t: number }
  | { kind: 'started'|'finished'; label: string; t: number };
...
export class AppComponent {
  ...
  async run() {
    ...
    try {
      ...
      const stop = this.svc.openEvents(runId, (ev) => this.onEvent(ev));
      // when finished, load final results
      const poll = setInterval(async () => {
        ...
        if (finished) {
          ...
          try {
            ...
            // ignore if result not found (shouldn't happen)
          }
          this.busy.set(false);
        }
      }, 400);
